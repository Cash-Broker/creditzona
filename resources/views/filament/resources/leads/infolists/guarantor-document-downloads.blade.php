                ...
                <div class="flex min-w-0 items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            {{ $document['is_available'] ? 'Прикачен документ към поръчителя' : 'Файлът в момента не е достъпен' }}
                        </div>
                    </div>
                </div>

                @if ($document['is_available'])
                    <a href="{{ route('admin.leads.guarantors.documents.download', ['lead' => $record->lead_id, 'guarantor' => $record, 'path' => $document['path']]) }}"
